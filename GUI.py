...
class Programa:
    def __init__(self):
        ...
        btn_start.grid(column=1, row=4, sticky='ne', padx=(5, 5))
        btn_start = tk.Button(frm_5Tupla_coleta, text="teste", width=10, height=3, command=self.teste_insert)
        btn_start.grid(column=0, row=4, sticky='ne', padx=(5, 5))
        ...
        self.edits_5Tupla[4].configure(yscrollcommand=scrl_5Tupla.set)
        scrl_5Tupla.config(command=self.edits_5Tupla[4].yview)
        self.edits_5Tupla[4].pack(side=tk.LEFT)
    ...
    def teste_insert(self):
        ...
        self.edits_5Tupla[4].insert('1.0', 'q0, 0, q1\nq1, 1, q2\nq2, 1, q1\nq2, 0, q2\n')

    def inicia_prog(self):
        self.dados = {
            'Q': self.edits_5Tupla[0].get(),
            'E': self.edits_5Tupla[1].get(),
            'q': self.edits_5Tupla[2].get(),
            'F': self.edits_5Tupla[3].get(),
            'FT': self.edits_5Tupla[4].get('1.0', tk.END)
        }
        print('Pré tratamento:', self.dados)
        self.tratamento_dados()

    def tratamento_dados(self):
        self.dados = {
            'Q': self.dados['Q'].replace(',', '').split(),
            'E': self.dados['E'].replace(',', '').split(),
            'q': self.dados['q'].replace(',', '').split(),
            'F': self.dados['F'].replace(',', '').split(),
            'FT': [elem.replace(',', '').split() for elem in self.dados['FT'].split('\n') if elem != '']
        }


        print('Pós tratamento:', self.dados)
